    ...
    class YITH_Popup_Newsletter_Mailchimp {
        ...
        public function admin_enqueue_scripts(){
            ...
            if( get_post_type() == YITH_Popup()->post_type_name && ( strcmp( $pagenow, 'post.php' ) == 0 || strcmp( $pagenow, 'post-new.php' ) == 0 ) ){
                ...
                wp_localize_script('ypop-refresh-mailchimp-list', 'mailchimp_localization', array( 'url' => admin_url( 'admin-ajax.php'), 'nonce_field' => wp_create_nonce( 'yit_mailchimp_refresh_list_nonce' ), 'refresh_label' => __( 'Refreshing...', 'yith-woocommerce-popup' ) ) );
            }
        }
            ...
            if ( isset( $mail ) && is_email( $mail ) ) {
                if ( isset( $list ) && strcmp( $list, '-1' ) != 0 && isset( $apikey ) && strcmp( $apikey, '' ) != 0 && check_ajax_referer( 'yit_mailchimp_newsletter_form_nonce', 'yit_mailchimp_newsletter_form_nonce', false ) ) {
                    ...
                    if ( $result['error_count'] != 0 ) {
                        ...
                        _e( '<span class="success">Email successfully registered</span>', 'yith-woocommerce-popup' );
                    }
                    die();
                }
                else {
                    _e( '<span class="error">Ops! Something went wrong</span>', 'yith-woocommerce-popup' );
                    die();
                }
            }
            else {
                _e( '<span class="notice">Ops! You have to use a valid email address</span>', 'yith-woocommerce-popup' );
                die();
            }
        }
